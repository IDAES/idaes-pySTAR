...
def constant_fit(es, cst_count, X, y, opt, tree):
    ...
    if cst_count > 0:
        ...
        for i in range(len(y)):
            ...
            if (
                pyomo_es_eval is None
                or "inf" in str(pyomo_es_eval[i])
                or "nan" in str(pyomo_es_eval[i])
            ):
                print("Tree is not valid")
                return None, 999999, 0
            if (
                "/(0)" in str(pyomo_es_eval[i])
                or "/(0*" in str(pyomo_es_eval[i])
                or "/(0/" in str(pyomo_es_eval[i])
            ):
                print("Tree is not valid")
                return None, 999999, 0
        ...
        try:
            results = opt.solve(
                cm, tee=True, keepfiles=True, symbolic_solver_labels=True
            )
