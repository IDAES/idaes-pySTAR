...
y, #target values
    ...
    level = {n: int(np.ceil(np.log2(n+1)))-1 for n in N} # dictionary level(n)
    max_level = max(level.values())  #or max_level=depth
    ...
    m.d = Var(domain=PositiveIntegers, bounds=(1, max_level))  # depth the solver picks
    
    def depth_envelope(m, n):
        return m.d >= level[n] * sum(m.y[(n, o)] for o in O if (n, o) in Y)

    m.depth_envelope = Constraint(N, rule=depth_envelope)
    ...
    def bic(m):
        ssr = sum((y[i - 1] - m.v[i, 1]) ** 2 for i in range(1, n_data + 1))
        # 1)Penalize the number of operators
        # p = sum(m.y[(n, o)] for n in NnotT for o in (B + U) if (n, o) in Y)

        # 2)Penalize the number of nodes
        #p = sum(m.y[(n, o)] for n in N for o in O if (n, o) in Y)
        
        #3) Penalize the number of operators with weights
        #weights = {'+':1, '-':1, '*':1, '/':2, '**0.5':2, 'log':2, 'exp':2}
        #p = sum(weights[o] * m.y[(n, o)] for n in NnotT for o in (B + U) if (n, o) in Y)
        
        # epsilon is used to avoid numerical issues when ssr is close to 0
        return n_data * log((ssr + eps) / n_data) + m.d * log(n_data)

    m.obj = Objective(rule=bic, sense=minimize)
    ...
    return m, Y, B, U, L, NnotT, T, N, c_lo, c_up, m.d
