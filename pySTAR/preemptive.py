...
df = pd.read_csv(
    os.path.join(SCRIPT_DIR, "Datasets/Feynman_all_depths", dataset_name),
    sep="\t",
)
X_df = df.iloc[:, :-1]
y_df = df.iloc[
...
if __name__ == "__main__":
    ...
    # Save solution from first optimization for warm-starting the second
    previous_solution = {}
    for var in mdl.component_data_objects(pyo.Var):
        if var.value is not None:
            previous_solution[var.name] = var.value
    print(
        f"\nSaved {len(previous_solution)} variable values for warm-start in next optimization."
    )
...
if __name__ == "__main__":
    solver = solver_name
    mdl = build_model()
    mdl.constrain_sse(sse_true)

    # Update experiment name for second optimization (lexicographic nodes minimization)
    fitness_metric = "lex_nodes"
    experiment_name = f"{dataset_name}_{model_type}_lex_nodes_{solver_name}_max_time_{max_time}_maxdepth{depth}_samples{number_of_samples}"
    if custom_bounds:
        experiment_name += "_bounds_x10"

    # Create new train/test data subdirectories and save base data with new experiment name
    train_data_subdir_name = f"train_{model_type}_lex_nodes_{solver_name}_max_time_{max_time}_maxdepth{depth}_samples{number_of_samples}"
    train_data_subdir_path = os.path.join(
        RESULTS_DIR, "Train_data", train_data_subdir_name
    )
    os.makedirs(train_data_subdir_path, exist_ok=True)

    test_data_subdir_name = f"test_{model_type}_lex_nodes_{solver_name}_max_time_{max_time}_maxdepth{depth}_samples{number_of_samples}"
    test_data_subdir_path = os.path.join(
        RESULTS_DIR, "Test_data", test_data_subdir_name
    )
    os.makedirs(test_data_subdir_path, exist_ok=True)
    ...
    # Warm-start: initialize with solution from the first optimization
    if previous_solution:
        ...
        for var in mdl.component_data_objects(pyo.Var):
            if var.name in previous_solution:
                ...
                warm_start_count += 1
        # Compute aux_var_nodes from warm-started select_node values, since
        # this variable only exists in the "nodes" objective (not in the first model)

        print(
            f"Warm-start: initialized {warm_start_count} variables from first optimization.\n"
        )
